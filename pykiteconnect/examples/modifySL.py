...
kite.set_access_token("AfjFF6Fv0eXcc8D2hYjSvlieBmAGT3GF")
...
def placeHedge():
    ...
    kite.place_order("regular", kite.EXCHANGE_NFO, 'BANKNIFTY' + expiry + str(peStraddleStrike) + "PE",
                     ...
                     kite.PRODUCT_MIS, kite.ORDER_TYPE_LIMIT, price=round(peStraddlePrice + 0.5, 1),
                     validity=kite.VALIDITY_DAY,
                     disclosed_quantity=0)
    kite.place_order("regular", kite.EXCHANGE_NFO, 'BANKNIFTY' + expiry + str(ceStraddleStrike) + "CE",
                     kite.TRANSACTION_TYPE_BUY, quantity,
                     kite.PRODUCT_MIS, kite.ORDER_TYPE_LIMIT, price=round(ceStraddlePrice + 0.5, 1),
                     validity=kite.VALIDITY_DAY,
                     disclosed_quantity=0)
...
def placeStraddle(timehr=None, timemin=None, timesec=None):
    ...
    placeOptionOrderMarket('BANKNIFTY' + expiry + str(ATMSTraddleVal["PE"]["StrikePrice"]) + "PE",
                           kite.TRANSACTION_TYPE_SELL, quan=quantity)
...
def main(hedge, straddle, token, expiry):
    kite.set_access_token(token)
    expiry = expiry
    datetime_object = datetime.datetime.now()
    print(datetime_object)
    if hedge:
        print("Sleeping for hedge")
        # sleepUntil(None, 17, 58)
    #     print("woke up for hedge")
    #     placeHedge()
    # datetime_object = datetime.datetime.now()
    # print(datetime_object)
    # if straddle:
    #     placeStraddle(None, 19, 0)
    datetime_object = datetime.datetime.now()
    print(datetime_object)
    trailingSL()
